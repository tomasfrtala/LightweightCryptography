...
#include <Arduino.h>
#include "EEPROM.h"
#include "src/present/present80.h"
...
void setup()
{
    ...
    time = micros(); // time start
    present80_init(key, KEYLENGTH, &key_init);
...
//        for (int piece = 0; piece < N_BLOCK; piece += N_BLOCK, pPlain += N_BLOCK)
	...
	//	time = micros(); // time start
//	present80_enc(cipher, &key_init);
	present80_enc(pPlain, &key_init);
	//	emit = micros(); // time end
