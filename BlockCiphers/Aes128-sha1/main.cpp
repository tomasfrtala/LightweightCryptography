...
//        for (int piece = 0; piece < N_BLOCK; piece += N_BLOCK, pPlain += N_BLOCK)
	...
	//	time = micros(); // time start
	//	aes128_enc(cipher, &key_init); // Encrypt message
	aes128_enc(pPlain, &key_init); // Encrypt message
	//	emit = micros(); // time end
    ...
    byte hash[20];
    hmac_sha1(hash, key, 128, plain, 8192);
    // ------------------------------------------
    emit = micros(); // time start
