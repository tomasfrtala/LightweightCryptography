...
#include <Arduino.h>
#include "EEPROM.h"
#include "src/trivium/trivium.h"
#include "src/hmac/hmac-sha1/hmac-sha1.h"
...
void setup()
{
    ...
    time = micros(); // time start
    trivium_init(key, 80, vectorIV, 80, &reg);
    //    trivium_init(key, 80, vectorIV, 80, &reg2);
...
//    for (int piece = 0; piece < DATALENGTH; piece++)
	...
	plain[piece] ^= trivium_getbyte(&reg);
...
//    hmac_sha1(hash, key, 80, plain, 8192);
    hmac_sha1(hash, key, 80, plain, 8);
    // ------------------------------------------
    emit = micros(); // time start
    ...
    //    for (int piece = 0; piece < DATALENGTH; piece++)
    //    {
    //	plain[piece] ^= trivium_getbyte(&reg2);
